...
Il utilise les résultats de l'extraction faite avec extract_pdf_text.py
...
def extract_scan_tables(df_raw_text_data: pd.DataFrame, scan_output_dir: Union[str, Path]) -> List[Dict]:
...
def main(output_dir: Path):
    ...
    if not raw_text_csv.exists():
        raise ValueError(f"Fichier de l'extraction des textes bruts '{PDF_RAW_TEXT_CSV_FILE}' inexistant.\n"
                         ...
                         f"'extract_pdf_text.py', avec comme dossier de sortie {output_dir}")
    ...
    data = extract_scan_tables(df_raw_text_data, scan_tables_images_dir)
